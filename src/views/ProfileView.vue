...
// (!!! 修复 TS 错误 !!!)
// selectedAvatar 必须在 user.id 可用后才能初始化
...
watch(
  ...
  (newId) => {
    if (newId) {
      // (!!! 修复 TS 错误 !!!)
      // 现在 newId 确认是 number, localStorageKey.value 也是安全的
      ...
      if (savedAvatar && availableAvatars.value.includes(savedAvatar)) {
        ...
        selectedAvatar.value = availableAvatars.value[availableAvatars.value.length - 1];
      }
    }
  },
...
const cultureTestResultTitle = computed(() => {
  // testResult.value 已经是 TestResult | null
  // ?. 可选链确保了安全
